...
export default function CreateOrder() {
  ...
  const submit = async (e) => {
    ...
    const payloadAssignments = filteredItems.map((item, idx) => {
      const a = assignments[idx] || {};
      return {
        costBreakdownItemId: null,
        employeeId: a.employeeId && a.employeeId !== "__none" ? a.employeeId : null,
        taskName: item.name,
        status: "pending",
      };
    });
      ...
      >
        <div className="grid gap-6 md:grid-cols-2">
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-3">
              <Label className="text-sm font-medium text-zinc-700">Employee Assignments</Label>
              <div className="space-y-2 rounded-xl border border-zinc-100 bg-zinc-50/50 p-4">
                {form.items
                  .filter((it) => it.name.trim() !== "")
                  .map((item, idx) => {
                    const originalIdx = form.items.findIndex(
                      (f) => f.name === item.name && f.cost === item.cost,
                    );
                    const assignment = assignments[originalIdx] || {};
                    return (
                      <div key={originalIdx} className="flex items-center gap-3">
                        ...
                        <div className="w-48">
                          <Select
                            value={assignment.employeeId || "__none"}
                            onValueChange={(val) => {
                              setAssignments((prev) => {
                                const next = [...prev];
                                next[originalIdx] = {
                ...
                {form.items.filter((it) => it.name.trim() !== "").length === 0 && (
                  <p className="text-xs text-zinc-500">Add cost items above to assign employees.</p>
                )}
              </div>
            </div>
          </div>
