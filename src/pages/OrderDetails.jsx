...
export default function OrderDetails() {
  ...
  const fetchAssignments = useCallback(async () => {
    try {
      const { data } = await api.get(`/orders/${id}/assignments`);
  ...
  const handleSaveCosts = async () => {
    ...
    setSavingCosts(true);
    try {
      const payload = { items: filteredItems };

      if (assignmentForm.length > 0) {
        payload.assignments = assignmentForm.map((a) => ({
        ...
        items={[
          {
            ...
            subtitle: (order.paymentStatus || "unpaid").replace("_", " "),
            icon: DollarSign,
          ...
          {
            title: "Total Costs",
            value: `Rs ${order.totalCost.toLocaleString()}`,
            subtitle: `${order.items?.length || 0} production items`,
            icon: Receipt,
          ...
          {
            title: "Net Profit",
            value: `Rs ${order.profit.toLocaleString()}`,
            subtitle: `${((order.profit / (order.price || 1)) * 100).toFixed(1)}% margin`,
            icon: TrendingUp,
            iconColor: order.profit >= 0 ? "text-emerald-600" : "text-red-600",
            borderColor: order.profit >= 0 ? "border-l-emerald-500" : "border-l-red-500",
            trend: order.profit >= 0 ? "up" : "down",
            trendValue: `${((order.profit / (order.price || 1)) * 100).toFixed(1)}%`,
          },
        ]}
      />
      ...
      <Tabs value={activeTab} onValueChange={onTabChange} className="w-full">
                  ...
                  setAssignmentForm(
                    (order.items || []).map((item) => {
                      const existing = assignments.find(
                        (a) => String(a.costBreakdownItemId) === String(item._id),
                      );
                      return {
                        costBreakdownItemId: item._id,
                        employeeId: existing?.employeeId?._id || existing?.employeeId || "",
                        taskName: existing?.taskName || item.name,
                  ...
                  <div className="space-y-3 rounded-xl border border-zinc-100 bg-zinc-50/50 p-4">
                    {costForm.map((item, idx) => (
                      <div key={idx} className="flex flex-wrap gap-2">
                        ...
                        <div className="w-40">
                          <Select
                            value={assignmentForm[idx]?.employeeId || "__none"}
                            onValueChange={(val) => {
                              setAssignmentForm((prev) => {
                                ...
                                next[idx] = {
                                  ...(next[idx] || {}),
                                  costBreakdownItemId: item._id,
                                  employeeId: val === "__none" ? "" : val,
                                  taskName: item.name,
                                  status: next[idx]?.status || "pending",
                                };
                                return next;
                              });
                            }}
        ...
        <TabsContent value="assignments">
          <Card>
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-4">
              <CardTitle className="text-base flex items-center">
                <User className="mr-2 h-4 w-4 text-zinc-400" /> Employee Assignments
              </CardTitle>
            </CardHeader>
            <CardContent>
              <div className="rounded-lg border border-zinc-100 overflow-hidden shadow-sm">
                  ...
                  <tbody className="divide-y divide-zinc-100">
                    {assignments.length > 0 ? assignments.map((a) => (
                      <tr key={a._id} className="hover:bg-zinc-50/50">
                        <td className="px-4 py-2 text-zinc-700">{a.taskName}</td>
                        ...
                        <td className="px-4 py-2 text-center">
                          <Badge className="capitalize text-xs" status={a.status}>
                            {a.status}
                          </Badge>
                        </td>
                      </tr>
